...
void TaskScheduler::Join() {
	...
	// the least actionable failure there is.
	{
		...
		while (std::chrono::steady_clock::now() < deadline) {
			...
			for (size_t i = 0; i < loPri.size() && idle; ++i)
				if (loPri[i]->size() != 0 || hiPri[i]->size() != 0) idle = false;
...
static std::atomic<size_t> g_awakeFloor{ 2 };
...
static std::atomic<size_t> g_awakeFloorBase{ 2 };

size_t TaskScheduler::GetAwakeFloorBase() noexcept {
	return g_awakeFloorBase.load(std::memory_order_relaxed);
}
...
void TaskScheduler::SetAwakeFloor(size_t k) noexcept {
	...
	g_awakeFloorBase.store(k, std::memory_order_relaxed);
	const size_t prev = g_awakeFloor.exchange(k, std::memory_order_relaxed);
	...
	for (size_t i = prev; i < k && i < instance->workers.size(); ++i) {
		if (Thread* w = instance->workers[i])
			w->NotifyWorker(/*force*/ true);
	}
}
size_t TaskScheduler::GetAwakeFloor() noexcept         { return g_awakeFloor.load(std::memory_order_relaxed); }
...
void TaskScheduler::RedistributeToOverflow(size_t ownerIdx, size_t count) {
	const size_t baseF = g_awakeFloorBase.load(std::memory_order_relaxed);
	const size_t liveF = g_awakeFloor.load(std::memory_order_relaxed);
	...
	for (size_t i = 0; i < count; ++i) {
		...
		auto opt = loPri[ownerIdx]->pop_bottom();
		if (!opt) break;
		Task* t = *opt;
		if (!t) break;
...
void TaskScheduler::ForceAwakeFloorToBase() noexcept {
	g_awakeFloor.store(g_awakeFloorBase.load(std::memory_order_relaxed), std::memory_order_relaxed);
}
void TaskScheduler::ResetAwakeFloorPeak() noexcept {
	g_awakeFloorPeak.store(g_awakeFloor.load(std::memory_order_relaxed), std::memory_order_relaxed);
}
...
bool TaskScheduler::CollapseAwakeFloorToBase() noexcept {
	const size_t base = g_awakeFloorBase.load(std::memory_order_relaxed);
	const size_t cur  = g_awakeFloor.load(std::memory_order_relaxed);
	...
	const long long now = MonotonicNs();
	if (now - g_lastFloorGrowNs.load(std::memory_order_relaxed) < kFloorHoldNs) return false;
	...
	size_t expected = cur;
	return g_awakeFloor.compare_exchange_strong(expected, base, std::memory_order_relaxed);
}
...
static constexpr size_t kFloorGrowCap = 16;
...
void TaskScheduler::NoteFloorCrowding(size_t submitted) noexcept {
	...
	const size_t prev = g_awakeFloor.exchange(want, std::memory_order_relaxed);
	...
	if (want <= prev) return;
	for (size_t i = prev; i < want && i < n; ++i)
...
size_t TaskScheduler::GetHotWorkers() {
	...
	return g_hotWorkers.load(std::memory_order_relaxed);
}
...
bool TaskScheduler::HotScalingActive() noexcept {
	// DISABLED FOR NOW -- deliberately, and this is the one line that turns it off.
	//
	// The controller steers K from LANE OCCUPANCY: the fraction of wall time a hot worker spent
	// actually running lane tasks. That signal assumed an idle worker BLOCKS, so idle time was
	// unambiguous. With the blocking park gone an idle worker spins, and the measurement it is
	// reading no longer means what the controller was tuned against. Symptom: dynamic_k_test began
	// failing about one run in four while printing "ALL CHECKS PASSED" -- a controller decision that
	// went the wrong way on a threshold, not a crash.
	//
	// Turning it off HERE rather than at the call sites keeps the range API working and unchanged,
	// so nothing that configures it needs editing and re-enabling is this line. Static K is
	// unaffected: SetHotWorkers still pins K exactly where it is asked to.
	//
	// RE-TUNE BEFORE RE-ENABLING. The occupancy definition has to account for spin, or the
	// controller needs a different signal entirely.
	return false;
	...
	if (workers.size() <= kMaxHintQueues
		...
		if (const size_t baseF = GetAwakeFloorBase()) {
			...
			for (size_t w = 0; w < nWords; ++w) {
				...
				if (baseF <= lo)              keep = 0ull;
				else if (baseF - lo >= 64)    keep = ~0ull;
				else                          keep = (1ull << (baseF - lo)) - 1ull;
