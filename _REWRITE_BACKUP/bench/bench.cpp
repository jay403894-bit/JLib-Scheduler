...
static void BenchLatency(JLib::TaskScheduler& sched) {
    ...
    // placement is still routing to sleepers and the floor is decoration.
    {
        ...
        printf("               landed on the floor [0,%zu): %u of %u (%.1f%%)  -- the rest paid a wake\n",
            floorN, onFloor, total, total ? 100.0 * (double)onFloor / (double)total : 0.0);
        if (total) {
...
int main(int argc, char** argv) {
    ...
    bool   floorGrowth = true;   // `nogrow` turns the growth controller off  // floor=N; default = the shipped one
...
#define JLIBSCHED_VERSION "unknown"   // hand-built outside CMake
    ...
    // SAY WHEN THE KNOB IS INERT. GetHotWorkers() currently returns 0 unconditionally -- the lane is
    // being deleted in phases, and the early return is the "provably unreachable" step. Until that
    // finishes, `hot=N` is accepted and does nothing, and a banner reading `hot=0` next to a command
    // line reading `hot=5` looks like the argument failed to parse rather than like a stubbed
    // feature. An inert knob has to announce itself; silence here costs an hour of bisecting.
    if (hotWorkers != bannerK) {
        printf("        NOTE: hot=%zu was requested but the LANE IS CURRENTLY STUBBED OFF\n"
               "              (GetHotWorkers() returns 0 unconditionally, mid-deletion), so K=0 and\n"
               "              the lane rows below skip themselves. This is not an argument error.\n",
               hotWorkers);
    }
    ...
    printf("        awake-floor=%zu  -- workers that never park (floor=N). SEPARATE FROM hot=N:\n"
           "                        hot=K picks the QUEUE a hiPri push lands in; the floor decides\n"
           "                        whether landing there costs an OS wake.\n",
           JLib::TaskScheduler::GetAwakeFloor());
...
static void BenchRecursiveForkJoin(JLib::TaskScheduler& sched) {
    ...
    for (int run = 0; run < kRuns; ++run) {
        ...
        }, false, JLib::FiberSize::Standard, JLib::TaskType::Fiber);  // suspends, so it needs a fiber
