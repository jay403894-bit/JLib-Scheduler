...
//
// The receive loop is a FIBER TASK on the worker pool. It parks on the reactor between datagrams
// and occupies NO thread while it waits -- the window's message pump has the main thread to itself,
// and the workers stay free for whatever else the app is doing. The loop touches no UI state: it
...
#include "IoReactor.h"
#include "IoFiber.h"        // Fib::RecvFrom -- the replacement for co_await, and no C++20 needed
...
// A FUNCTION POINTER AND A CONTEXT, NOT A LAMBDA, and that is a hard requirement rather than a
// style choice: a lambda task is always Native, because its closure lives in a slab slot that
// cannot own a fiber row. This loop must SUSPEND, so it must be a Fiber, so it must be a plain
// `void(void*)` with its state passed alongside.
struct RecvLoopCtx {
...
// THE FIBER STACK IS THE COROUTINE FRAME. This used to be a `JLib::Coro` and `co_await
// RecvFromAsync`; the reactor is unchanged underneath, and Fib::RecvFrom is the same wait written
// for a fiber -- which needs no language feature, because a fiber already knows how to suspend.
//
// What the reactor asks of the caller is identical either way: it holds pointers to `from` and to
// the buffer ACROSS the wait, so both have to live somewhere that survives a suspension. In the
// coroutine that was the frame. Here it is the fiber's own stack, which is exactly as durable --
// a suspended fiber keeps its stack, that is what a fiber IS. So these stay ordinary locals, and
// the socket test's statics remain a workaround for lambdas that a real loop should not copy.
static void RecvLoopBody(void* p) {
    auto& c = *static_cast<RecvLoopCtx*>(p);
    ...
    for (;;) {
        // FRESH PER OPERATION. IoRequest is the reactor's per-in-flight-request record and IoResult
        // is where it writes the answer; reusing one across two waits would let a cancelled request
        // still in the kernel's hands scribble on the next one's result.
        JLib::IoRequest req{};
        JLib::IoResult  out{};
        const JLib::IoResult r =
            JLib::Fib::RecvFrom(*c.sched, c.sk, buf, sizeof buf, &from, req, out, 0, c.tok);
        ...
        ::PostMessage(c.hwnd, kWmPkt, 0, 0);
    }
    // Returning is the whole teardown: the task completes, its WaitGroup drops to zero, and the
    // WaitFor in RunReceiver returns. There is no co_return because there is no coroutine.
}
...
int RunReceiver() {
    ...
    // THE CONTEXT'S SCOPE MUST COVER THE WAIT. `ctx` is a local of this function and the loop reads
    // it on every iteration, so it has to outlive the task -- which the WaitFor below is what
    // guarantees. Declare-then-join in the same function satisfies that structurally; a context
    // declared in a narrower scope than its join does not, and the failure is a use-after-scope that
    // usually still runs.
    RecvLoopCtx ctx{ &sched, s, hwnd, scope.Token() };

    // EXPLICITLY A FIBER. It suspends on the reactor, and only a fiber can. CreateTask already
    // defaults to Fiber, but a sample is read for what it teaches, and "this had to be a fiber" is
    // one of the things it teaches.
    wg.n.store(1, std::memory_order_relaxed);
    JLib::Task* loop = sched.CreateTask(&RecvLoopBody, &ctx, JLib::Lane::Normal,
                                        JLib::TaskType::Fiber, JLib::CorePref::Default);
    if (!loop) {
        std::printf("CreateTask failed\n");
        return 1;
    }
    loop->waitGroup = &wg;
    sched.Push(loop);
