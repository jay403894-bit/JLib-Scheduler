...
#include "../IoPlatform.h"                // the seam IoShared.cpp calls into
...
namespace JLib {
    ...
    // THE SEAM'S WINDOWS HALF, declared in src/IoPlatform.h. It is defined HERE and not in the
    // shared file because only a backend knows where the descriptors sit inside `native` -- after
    // the OVERLAPPED on Windows, after the msghdr on POSIX. Each backend defines this symbol over
    // its own layout and neither has to know the other's.
    //
    // A thin forward to the file-static above rather than a second copy of the loop: this file's
    // own Submit paths already call FillBufs directly, and two bodies that must agree about a
    // memory layout is exactly the duplication the hoist exists to remove.
    namespace ioplat {
        ...
        void Run() {
            ...
            const auto Flush = [&](Task** hi, std::size_t& nh, Task** lo, std::size_t& nl) {
                ...
                auto pushSteered = [&](Task** arr, std::size_t n, Lane lane) {
                    ...
                    // ---- THE INTAKE DECIDES FIRST, BEFORE ANYTHING IS COMPUTED FOR STEERING -----
                    //
                    // ORDER IS THE POINT. This sat BELOW the candidate scan, so every flush loaded
                    // LaneBacklogMask and counted the free reserved workers -- and then returned
                    // here without using either. Real work on the completion path, on the default
                    // configuration, for a decision already made. Steering state is only computed
                    // now when the control arm is actually selected.
                    //
                    // The intake needs none of it: it does not choose a worker, which is the whole
                    // reason it fixes the strands. Asking which workers are free is a question only
                    // the push-time binding below has to answer.
                    if (TaskScheduler::LaneIntakeEnabled()) {
                        ...
                        // K == 0: no consumer for the shared queue, so the floor is the only place
                        // with one. Refusing rather than enqueueing is what keeps a queue nobody
                        // reads from becoming the stranding failure in a new shape.
                    ...
                    // ---- BELOW HERE IS THE CONTROL ARM: push-time binding, as it was ------------
