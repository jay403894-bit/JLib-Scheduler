...
"            on wake latency and ~2x worse on the frame DAG -- BUT READ THE DATE.\n"
"            That run predates the AWAKE FLOOR, which shipped 4.0.1 on 2026-08-27.\n"
"            Every worker parked when idle then, so every dispatch paid a wake and\n"
"            a pinned worker that could not migrate paid it worse -- which is the\n"
"            exact mechanism the number measures. With the floor defaulting to 1-2\n"
"            today the threads that matter never park, so the penalty's mechanism\n"
"            is largely gone and the figure should not be quoted as current.\n"
"            What replaces it is a re-run, not an assumption in either direction:\n"
"            `hard` against `ideal` at the same floor. Nobody has done it.\n"
