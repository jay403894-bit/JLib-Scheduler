...
static void BenchLatency(JLib::TaskScheduler& sched) {
    ...
    if (stallCount) {
        ...
        printf(""
               ...
               "                 pool   -> the waiter spun the whole time and kept seeing n>0, so\n"
               "                           the WAITER was not the late one. BY ELIMINATION, not a\n"
               "                           finding: this does NOT establish a scheduler defect, and\n"
               "                           it used to claim it did. A trip can land here with the\n"
               "                           target worker simply OFF-CPU, or with the time spent in\n"
               "                           COMPLETION after the body already ran -- neither is\n"
               "                           something placement or the permit word can fix.\n"
               "                           SCHEDULER-IMPLICATED is the positive test (dispatch\n"
               "                           dominated AND the target made idle passes while the work\n"
               "                           sat there -- it was searching and missed it). WHEN THE\n"
               "                           TWO DISAGREE, BELIEVE THE IMPLICATED COUNT: `pool 1` next\n"
               "                           to `IMPLICATED 0` means look at the trip's own idle-pass\n"
               "                           line, which will read 0.\n"
...
"\n"
"usage: SchedulerBench [ideal|hard|none|physical] [poolSize] [FLAGS...]\n"
"\n"
"MOST OF WHAT FOLLOWS IS AN A/B ARM, not a setting to adopt. Where a default is\n"
"stated, that default is the shipped behaviour and the flag exists to turn it OFF\n"
"so both arms can be compared in one session on one machine.\n"
"\n"
...
"  hard      bind each worker to one logical CPU. Measured 2026-08-05 at ~45%% worse\n"
"            on wake latency -- BUT READ THE DATE. That run predates the AWAKE\n"
"            FLOOR (4.0.1, 2026-08-27), when every worker parked and every dispatch\n"
"            paid a wake. Re-measured 2026-09-03: at floor=0, `hard` costs 50-136x\n"
"            more STALLS than `ideal` (291-815 vs 6 per 20,000) with p50 nearly\n"
"            unchanged -- so the cost is a TAIL, and the percentile framing was\n"
"            always the wrong description. At the default floor=2 the difference is\n"
"            gone: 5 stalls vs 5.\n"
...
"            is a DIAGNOSTIC -- do not ship a small pool, it starves everything\n"
"            that is not a tiny graph.\n"
"  nosweep   skip the ParallelFor crossover and splitter-vs-cursor sweeps, the two\n"
"            slowest sections. A pool-size sweep then costs seconds, not minutes.\n"
"  noev      skip latency/hot, event/resume and io-pipe -- the only sections that\n"
"            exercise the lane and the Event primitives, and so the only reason\n"
"            hot=N changes anything but the compute pool size. They RUN by default;\n"
"            a later `ev` on the same command line turns them back on.\n"
"  stall=N   microseconds a round trip must exceed to be reported as pathological.\n"
...
"            N comes out of the compute pool, so raising it makes throughput\n"
"            slightly worse; latency/hot and io-pipe are what you get back.\n"
"            hot=0 COLLAPSES the lane: a lane push routes to loPri instead, because\n"
"            nobody probes the lane at K=0, and latency/hot and io-pipe then SKIP\n"
"            themselves rather than printing latency/cold twice under a new name.\n"
"  floor=N   the AWAKE FLOOR: workers 0..N-1 never park (SetAwakeFloor). DEFAULT is\n"
"            CHOSEN AT Init FROM THE POOL SIZE -- 1 on a pool of 8 or fewer, 2\n"
"            above -- not 0. `floor=auto` skips the call so the library default\n"
"            applies. THIS IS NOT hot=N: hot picks WHICH QUEUE a push lands in,\n"
"            floor decides whether landing anywhere costs an OS wake.\n"
"  neverpark RESERVATION DOES NOT IMPLY NEVER-PARK, and this is the flag that makes\n"
"            it so (SetReservedNeverParks). Without it the banner will tell you\n"
"            'K IS NOMINAL: N of N workers in [0,K) PARKED' -- parkable cores with\n"
"            a reserved name. ANY band-placement experiment needs this or it is\n"
"            testing nothing. Costs ordinary latency ~35-43%% and buys io-pipe p99\n"
"            ~3.6x; that trade is the point, not a defect.\n"
"  resv=N    workers reserved for the lane ONLY (SetReservedHiPri). 0 disables\n"
"            reservation entirely, so ordinary work may use every worker. The A/B\n"
"            for 'what does a reserved core cost when no I/O is arriving'.\n"
"  hotprio   raise thread priority per BAND -- reserved to TIME_CRITICAL, the base\n"
"            floor to HIGHEST, everything else Normal. Elevating the WHOLE pool\n"
"            measured 5x worse, so this is per-band on purpose: do not read a win\n"
"            here as 'elevate more'.\n"
"\n"
"=== PLACEMENT EXPERIMENTS (all OFF by default, all experiments) ================\n"
"  hotpin    confine workers [0,K) to their own cores. MEASURED WORSE ALONE: burst\n"
"            p99 27ms against a 150us baseline, and pin+RT worse than RT alone.\n"
"            Pinning confines without excluding, so an OS thread landing on that\n"
"            core becomes a WAIT rather than a migration. Reproduce, do not adopt.\n"
"  hotexcl   the other half: every other thread masks those cores off. Implies\n"
"            hotpin. The userspace approximation of isolcpus -- it CANNOT exclude\n"
"            other processes, which is where it stops being the real thing.\n"
"  floorpin  widen that band from [0,K) to [0,K+Fbase), so the BASE floor is pinned\n"
"            and excluded too. Fbase is 1 or 2, fixed however wide the machine is,\n"
"            which is why this is NOT the whole-pool experiment. The GROWN floor is\n"
"            excluded on purpose -- F is a controller output, and pinning it would\n"
"            strand whatever the last burst grew. Measured 2026-09-03 at constant\n"
"            neverpark: no effect at any percentile.\n"
"  waiterpin pin the bench's own WAITER thread (ideal processor, not hard affinity\n"
"            -- a hint the OS may ignore, which is the right strength for a\n"
"            diagnostic). The stall report counts trips where the waiter sat on the\n"
"            core the body needed; this is the arm that tests whether moving it\n"
"            helps.\n"
"  narrowsteer  placement stops following the GROWN floor and steers only at the\n"
"            base (SetPlacementFollowsGrownFloor false). The A/B for whether\n"
"            steering at grown workers buys anything.\n"
"\n"
"=== THE FLOOR GROWTH CONTROLLER ===============================================\n"
"  nogrow    pin the floor at its base -- no push-side spill, no completion-side\n"
"            growth, no redistribute. The A/B for the whole growth controller, in\n"
"            ONE binary, so both arms can run in one session.\n"
"  growcap=N ceiling on how wide a BURST may grow the floor (SetAwakeFloorMax).\n"
"            NOT floor=N, which is a PERMANENT base and costs a 3.3-17us round trip\n"
"            at 31 against 0.6 at 2. A ceiling on a TRANSIENT burst is a different\n"
"            and far cheaper thing.\n"
"  floorhold=N  milliseconds a grown floor is held before it may shed. Default 6.\n"
"            The 'for how long' half of release-the-herd; sweep it against a row\n"
"            that produces work on a cadence, which is the case 6ms was never\n"
"            sized for.\n"
"  wakecost=N   nanoseconds, the cost every recruitment decision is a ratio\n"
"            against. Default 3000 (the measured OS wake here). Lower recruits\n"
"            sooner and wider; it is the mechanism's only tuned number.\n"
"\n"
"=== IDLE, SPIN AND PARKING ====================================================\n"
"  spinyield=N  how often an awake-floor worker yields while spinning\n"
"            (SetSpinYieldMask). Raising it while the floor can grow wide\n"
"            reproduces the permanent-floor=31 pathology from a default config.\n"
"            Sweep it against the SERIAL row, not just a burst row.\n"
"  yieldfloor=N  workers below this index never yield (SetYieldFloorMin). Set 0 to\n"
"            restore the old always-yield behaviour, which is this gate's A/B arm.\n"
"  relax=N   pause instructions a worker spins between polls (SetWorkerRelax).\n"
"            Raising it is a cheap way to make a worker LOOK asleep while it is\n"
"            still burning a core; a worker that wants a real backoff should park.\n"
"  park=cv | park=wait   which primitive a worker blocks on when it parks.\n"
"\n"
"=== PARALLELFOR ===============================================================\n"
"  mwidth / nomwidth   measured fan-out width, ON by default since 8-31. `mwidth`\n"
"            is kept so existing command lines and notes still work; `nomwidth` is\n"
"            the control arm, restoring the old iteration-count gate exactly.\n"
"  minfan=N  iterations per worker below which ParallelFor refuses to fan out. 0\n"
"            disables the gate entirely (pre-5.0.0 behaviour). Protects cheap\n"
"            bodies at small N and costs expensive ones at the same N -- so sweep\n"
"            it against the trivial and heavy rows TOGETHER, never one alone.\n"
"  leaves=N  leaf size for the recursive fork-join row.\n"
"  splitcap=N  cap on how far a range may split.\n"
"  norecruit turn range recruitment off (SetRangeRecruit false).\n"
"  memory    give the width probe genuinely distinct callable types, reproducing\n"
...
"            those push tasks individually, not as a batch.\n"
"\n"
"=== ACCEPTED AND IGNORED ======================================================\n"
"  sleep, nosleep, both -- these selected IdlePolicy, which was removed in 5.0.\n"
"  Still parsed so an old invocation runs instead of printing this text. `both` no\n"
"  longer re-execs the binary: there is no second park policy left to compare\n"
"  against. floor=<workers> is what `nosleep` used to mean.\n"
"\n"
"ONE RUN MEASURES ONE CONFIGURATION. The scheduler is a process-wide singleton and\n"
"both policy and pool size are fixed at Init(), so run the exe once per point --\n"
"and compare pastes from the same machine, idle, foregrounded, interleaved.\n");
    };
