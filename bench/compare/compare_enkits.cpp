...
static void BenchBulk(JLib::TaskScheduler& jl, enki::TaskScheduler& enki_) {
  ...
  if (g_doJ) {
    // ONE JLib row. There were two -- ParallelFor and ParallelRange -- from when those were separate
    // entry points with different mechanisms. ParallelRange was folded into ParallelFor in 1.4, so
    // the second row became the same call twice and reported a comparison it was not making.
    for (int r = 0; r < kRuns; ++r) {
        auto t0 = Clock::now();
